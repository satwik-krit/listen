...
DEVICE = torch.device("cpu") # Bypasses RTX 5060 Blackwell limitations
...
class MachineMelDataset(Dataset):
    def __init__(self, X_mel, stats):
        self.X = X_mel
        self.ch_min = np.array(stats["ch_min"], dtype=np.float32)
        self.ch_max = np.array(stats["ch_max"], dtype=np.float32)
...
def fast_loss(recon, original):
    # HACKATHON SPEED UP: Dropped SSIM entirely. Pure MSE is 50x faster.
...
def main():
    ...
    print("[ 1 / 3 ] Memory-Mapping X_mel.npy...")
    ...
    print("\n[ 2 / 3 ] Grouping by machine identity (Indices only)...")
    ...
    for machine_name, data in sorted(groups.items()):
        ...
        ch_min = [float(X_normal[:, c, :, :].min()) for c in range(3)]
        ch_max = [float(X_normal[:, c, :, :].max()) for c in range(3)]
        stats = {"ch_min": ch_min, "ch_max": ch_max}
        ...
        train_loader = DataLoader(MachineMelDataset(X_tr, stats), batch_size=BATCH_SIZE, shuffle=True)
        val_loader   = DataLoader(MachineMelDataset(X_val, stats), batch_size=BATCH_SIZE, shuffle=False)
        ...
        for epoch in range(EPOCHS):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                # Print progress on the same line to avoid spamming the console
                if batch_idx % 5 == 0:
                    print(f"      [Epoch {epoch+1:02d}/{EPOCHS}] Processing batch {batch_idx}/{len(train_loader)}...", end='\r')
            ...
            # Overwrite the batch progress line with the final epoch stats
            print(f"      [Epoch {epoch+1:02d}/{EPOCHS}] Train Loss: {train_loss:.5f} | Val Loss: {val_loss:.5f}" + " " * 20)
            
            if val_loss < best_val:
                best_val = val_loss
                best_threshold = np.percentile(val_scores, 99)
