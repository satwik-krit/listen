...
FAULT_MAP = {
    # ── Base Features ──────────────────────────────────────────────────
    "ZCR": "high frequency clicks or hissing — erratic friction, fluid leakage, or electrical arcing",
    "Mean Amplitude": "elevated overall noise energy — general mechanical wear or overload",
    "Amplitude Variance": "high energy fluctuation — unsteady operation, cavitation, or periodic impacts",
    "Median Pitch": "shift in fundamental frequency — motor speed deviation or structural resonance",
    "Spectral Centroid": "shift in frequency center — grinding, blade unbalance, or flow disruption",
    "Spectral Rolloff": "increase in high-frequency scraping — rail friction, dry bearing, or metal-on-metal contact",
    "MFCC 1": "abnormal spectral envelope — primary acoustic profile shift or casing resonance change",
    "MFCC 2": "harmonic profile distortion — secondary resonance shifts or complex mechanical rubbing",
    # ── Delta (Δ) Features (Rate of Change) ────────────────────────────
    "ΔZCR": "rapidly increasing hissing/clicks — accelerating friction or leak degradation",
    "ΔMean Amplitude": "increasing overall noise trend — progressive mechanical failure",
    "ΔAmplitude Variance": "worsening energy fluctuations — destabilizing mechanical operation",
    "ΔMedian Pitch": "drifting fundamental frequency — progressive motor failure or structural yielding",
    "ΔSpectral Centroid": "escalating high-frequency shift — worsening grinding or flow blockage",
    "ΔSpectral Rolloff": "rising high-frequency scraping — accelerated metal-on-metal wear",
    "ΔMFCC 1": "rapid shift in overall acoustic shape — imminent structural or casing failure",
    "ΔMFCC 2": "worsening harmonic distortion — accelerating complex mechanical degradation",
    # ── Fallback ───────────────────────────────────────────────────────
    "Healthy Operation": "No significant fault signature detected.",
}
...
def extract_features_from_signal(signal):
    ...
    return np.array(
        [rms, kurt, skw, peak, crest, mav, bpfo_ratio, bpfi_ratio], dtype=np.float32
    )
...
def predict_rul(raw_window, model, scaler):
    assert raw_window.shape == (50, 8), (
        f"Expected (50, 8), got {raw_window.shape}. "
        + f"Each row = one time snapshot, 8 features per row."
    )
    ...
    explanation = (
        f"Primary fault indicator: {top_name} (importance: {top_score:.3f}). "
        + "Suggests "
        + f"{fault_desc}. "
        + f"Bearing health at {rul_pct}% — status: {status}."
    )
    return {
