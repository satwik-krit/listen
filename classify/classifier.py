...
from config import MODEL_DIR_CLASSIFY

model_component = load(MODEL_DIR_CLASSIFY / "model_final.joblib")
model_valve = load(MODEL_DIR_CLASSIFY / "model_valve.joblib")
model_fan = load(MODEL_DIR_CLASSIFY / "model_fan.joblib")
model_pump = load(MODEL_DIR_CLASSIFY / "model_pump.joblib")
model_slider = load(MODEL_DIR_CLASSIFY / "model_slider.joblib")

component_models = {0: model_fan, 1: model_pump, 2: model_slider, 3: model_valve}
...
def predict(np_arr):
    ...
    y_comp = model_component.predict(
        np_arr
    )  # Extract the integer value from the numpy array
    if y_comp == 0:
