...
def load_healthy_features(data_dir: str) -> np.ndarray:
    feature_files = glob.glob(os.path.join(data_dir, "*_audio_features.npy"))
    if not feature_files:
        raise FileNotFoundError(f"No *_audio_features.npy files found in {data_dir}")

    all_features = []
    for path in feature_files:
        file_feats = np.load(path)
        if file_feats.ndim == 1:
            file_feats = file_feats.reshape(1, -1)
